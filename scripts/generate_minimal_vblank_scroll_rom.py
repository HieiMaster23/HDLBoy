#!/usr/bin/env python3
# =============================================================================
# Script:      generate_minimal_vblank_scroll_rom.py
# Description: Generate a no-MBC ROM that scrolls background from VBlank ISR
...
DEFAULT_OUTPUT = Path("roms/minimal_vblank_scroll.gb")
...
def append_io_write(program: list[int], address: int, value: int) -> None:
    program.extend([0x21, address & 0xFF, (address >> 8) & 0xFF])
    program.extend([0x36, value & 0xFF])
...
def append_visual_setup(program: list[int]) -> None:
    append_io_write(program, 0xFF40, 0x00)
    ...
    append_io_write(program, 0xFF47, 0xFC)
    append_io_write(program, 0xFF42, 0x00)
    append_io_write(program, 0xFF43, 0x00)
    append_io_write(program, 0xFF40, 0x91)
...
def build_program() -> bytes:
    ...
    append_visual_setup(program)

    program.extend(
        [
            0xAF,        # XOR A
            0xE0, 0x82,  # LDH ($82),A ; scroll counter in HRAM/debug overlay area
            0xE0, 0x0F,  # LDH ($0F),A ; clear IF
            0x3E, 0x01,  # LD A,$01
            0xEA, 0xFF, 0xFF,  # LD ($FFFF),A ; enable VBlank interrupt
            0xFB,        # EI
            0x76,        # HALT
            0x18, 0xFD,  # JR to HALT
        ]
    )
...
def build_vblank_handler() -> bytes:
    return bytes(
        [
            0xF0, 0x82,  # LDH A,($82)
            0x3C,        # INC A
...
def build_rom() -> bytearray:
    ...
    rom[VBLANK_VECTOR:VBLANK_VECTOR + len(build_vblank_handler())] = build_vblank_handler()
...
def main() -> None:
    ...
    print(f"VBlank handler bytes: {len(build_vblank_handler())}")
