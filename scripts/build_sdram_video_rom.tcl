# =============================================================================
# Quartus II Tcl Build Script - SDRAM Video ROM Top
# Usage: quartus_sh -t scripts/build_sdram_video_rom.tcl
# =============================================================================
...
puts "=== Starting SDRAM video ROM top compilation ==="
puts "Original top-level entity: $original_top"

set_global_assignment -name TOP_LEVEL_ENTITY sdram_video_rom_top
set_global_assignment -name SDC_FILE ../constraints/sdram_video_rom_timing.sdc
source ../constraints/sdram_pin_assignments.qsf

if {[catch {execute_flow -compile} result]} {
    puts "ERROR: SDRAM video ROM compilation failed - $result"
    set_global_assignment -name TOP_LEVEL_ENTITY $original_top
    catch {remove_global_assignment -name SDC_FILE ../constraints/sdram_video_rom_timing.sdc}
    project_close
    if {$qsf_original ne ""} {
...
puts "=== SDRAM video ROM compilation successful ==="
