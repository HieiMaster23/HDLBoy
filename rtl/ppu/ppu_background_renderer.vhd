...
-- Description: Minimal static background renderer using VRAM tile data/map
...
begin
    ...
    begin
        vram_addr <= tile_map_addr(pixel_x_reg, pixel_y_reg);
        fb_we <= '0';
        fb_addr <= fb_addr_reg;
        fb_data <= pixel_from_tile(tile_low_reg, tile_high_reg, pixel_x_reg);
        busy <= '0';
        done <= '0';
        ...
        case state_reg is
            ...
            when S_TILE_LOW_REQ | S_TILE_LOW_CAPTURE =>
                vram_addr <= tile_data_addr(tile_index_reg, pixel_y_reg, '0');
                busy <= '1';
            when S_TILE_HIGH_REQ | S_TILE_HIGH_CAPTURE =>
                vram_addr <= tile_data_addr(tile_index_reg, pixel_y_reg, '1');
                busy <= '1';
            when S_WRITE_PIXEL =>
                fb_we <= '1';
                busy <= '1';
